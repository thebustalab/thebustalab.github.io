...
ggplot(data = wine, aes(x = Dim.1, y = Dim.2)) + 
  geom_point(aes(fill = kmeans_cluster), shape = 21, size = 5) +
  ggrepel::geom_label_repel(aes(label = sample_unique_ID))

ggtree(wine, aes(color = kmeans_cluster), size = 2) + geom_tiplab() + coord_cartesian(xlim = c(0,50))
...
ggtree(wine, aes(color = kmeans_cluster), size = 2) + geom_tiplab()
...
ggplot(wine, aes(x = Dim.1, y = Dim.2, fill = kmeans_cluster)) + geom_point(shape = 21, size = 5)
```
...
Here are a few additional functions you may need.

```{r, message = FALSE}
## Let's load our CHEM5725 environment
source("https://thebustalab.github.io/R_For_Chemists/custom_functions/chem.R")

## Let's work with the built-in R database "Motor Trend Car Road Tests (mtcars)"
library(dplyr)
data(mtcars)  # load the data
head(mtcars,6)  # view the first 6 rows
?mtcars  # view the "help" with details on variables

## Now let's look at a few new functions:
...
# case_when() function -------------------------
# Classify the fuel efficiency of 
efficiency <- mutate(mtcars,fuel_efficient = case_when(
                     mpg <= 15 ~ "Bad", mpg > 15 & mpg < 25 ~ "ok", mpg >= 25 ~ "Good"))

# weighted.mean() ------------------------------
# In cases when the a variable of weights are provided.
# Common with survey data when a subset of the population
# is used to estimate the whole population. A weight is
# applied to "scale up" the survey size.

# population_estimate <- summarize(data, utility = weighted.mean(utility,weight))
population_estimate <- summarize(data, COSTELEC = weighted.mean(COSTELEC,HHWT))

# Review of filter, group_by, summarize, and %>%
