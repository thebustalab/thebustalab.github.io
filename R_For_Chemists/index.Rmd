...
```{r, echo = FALSE}
source("https://thebustalab.github.io/R_For_Chemists/custom_functions/runMatrixAnalysis.R", local = knitr::knit_global())
```
...
You will also want access to the information about which analytes are major contributors to each principal component. This can be determined using an ordination plot. This is important because it will tell you which analytes are associated with particular dimensions, and by extension, which analytes are associated with (and are markers for) particular groups in the PCA plot. Let's look at an example. We can obtain the ordination plot information using `runMatrixAnalysis()` with `analysis = "pca-ord"`:
...
head(per_small_pca_ord)
```

We can now visualize the ordination plot using our standard ggplot plotting techniques:

```{r, fig.align = "center", fig.height = 2, fig.width = 3}
library(ggforce) # Gives access to geom_circle
ggplot(per_small_pca_ord) +
	geom_segment(aes(x = 0, y = 0, xend = Dim.1, yend = Dim.2), size = 1) +
	geom_circle(aes(x0 = 0, y0 = 0, r = 1)) +
  theme_bw()
```

We also can access information about the how much of the variance in the dataset is explained by each principal component:
...
per_small_pca_dim <- runMatrixAnalysis(
...
1. The data sets and the `runMatrixAnalysis()` function can all be loaded into your R Session by running the command below:

```{r, message = FALSE}
source("https://thebustalab.github.io/R_For_Chemists/custom_functions/chem.R")
```

2. When you are filling out the `runMatrixAnalysis()` template, you can use the `colnames()` function to help you specify a long list of column names rather than typing them out by hand. For example, in the periodic table data set, we can refer to a set of columns (columns 10 through 20) with the following command:
...
1. Run a PCA analysis of the data (i.e. `runMatrixAnalysis()` with `analysis = "pca"`)
...
ggplot() +
  ...
  geom_label(data = filter(pca_ord, Dim.1 > 0.75, Dim.2 < 0.1, Dim.2 > -0.1), aes(x = Dim.1, y = Dim.2, label = sample_unique_ID), alpha = 0.4)
