...
If you are on Mac, you may need to now open your terminal and run the following two commands: `sudo chown -R `whoami`:admin /usr/local/bin` and then `~/Library/TinyTeX/bin/*/tlmgr path add`. 

On both Mac and PC, you then need to do:
```{r, message = FALSE, eval = FALSE}
...
Once that is complete, and assuming it is successful, attempt to load phylochemistry again:
```{r, message = FALSE}
source("http://thebustalab.github.io/phylochemistry/phylochemistry.R")
```
...
Now that we've got R, RStudio, and the `tidyverse` installed, we're going to look at a few core concepts in R.^[test]
...
Throughout your time with R, you will probably want help. I certainly do. In R, you can use a question mark `?` to get help with many different concepts. You can just put it in front of the thing you want help with. We'll see an example in just a minute...
...
In this set of exercises we're going to practice importing, filtering, and plotting data. We're going to work with two datasets: (i) [algae_data_data.csv](https://thebustalab.github.io/R_For_Chemists/sample_data/algae_data.csv) and (ii) [alaska_lake_data.csv](https://thebustalab.github.io/R_For_Chemists/sample_data/alaska_lake_data.csv). By clicking on those links you can download each dataset.
...
source("https://thebustalab.github.io/phylochemistry/phylochemistry.R", local = knitr:::knit_global())
```
...
```{r, message = FALSE}
# source("http://thebustalab.github.io/phylochemistry/phylochemistry.R", local = knitr:::knit_global())
source("http://thebustalab.github.io/phylochemistry/phylochemistry.R")
...
```{r, message = FALSE, fig.align = "center", eval = FALSE}
source("https://thebustalab.github.io/phylochemistry/phylochemistry.R")
AK_lakes <- read_csv("https://thebustalab.github.io/R_For_Chemists/sample_data/alaska_lake_data.csv")
head(AK_lakes)
...
```{r, fig.align = "center", fig.height = 3, fig.width = 4, message = FALSE, eval = FALSE}
source("https://thebustalab.github.io/phylochemistry/phylochemistry.R")
AK_lakes_pca_ord <- runMatrixAnalysis(
	data = AK_lakes,
	analysis = c("pca-ord"),
...
```{r, fig.height = 2.5, fig.width = 3, fig.align = "center", eval = FALSE}
ggplot(
...
```{r, fig.width = 8, fig.height = 4, message = FALSE, eval = FALSE}
source("https://thebustalab.github.io/phylochemistry/phylochemistry.R")
solvents_clustered <- runMatrixAnalysis(
  data = solvents,
  analysis = c("pca"),
...
```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), results="markup"}
knitr:::include_graphics('http://thebustalab.github.io/R_For_Chemists/figures/stats.png', dpi = NA)
```
...
Based on these tests, it looks like the data for aquifer 9 is significantly different from a normal distribution (Shaprio test p = 0.000008), and the means are certainly different from one another (Levene test p = 0.002).
...
The person that previously worked in your postion, Mr. Porter, recently completed a chemical analysis of the major ingredients that Fireside Brewing uses. However, Mr. Porter didn't have time to finish analyzing the data that was collected.

At Fireside Brewing, your boss is Mrs. Pilsner, a friendly woman who has worked in the brewing industry for decades and is a certified cicerone (a professional beer taster). As an expert taster, she is familiar with all the classes of compounds in beer ingredients and their corresponding flavors, but she is not an expert in chemistry or data analysis.
...
Put your answers in a powerpoint presentation (graphic in the slide, code in the notes section) and email it to Dr. Busta.
...
Mrs. Pilsner: *When I taste our beers made with hop extract, they don't taste very hoppy compared to our beers made with hop oil or hops. Why is this - how do all our different ingredients cluster according to their average chemical composition?*
...
To load this second data set, the hops data set, run our source() command. It will then be present in your environment as `hops_components`.

### Question 1

Mrs. Pilsner: *There are three major types of hops - aroma hops, dry hopping hops, and bittering hops. We had an emergency scenario last month where we ran out of bittering hops. We needed to substitute either aroma hops or dry hopping hops into the recipe in place of bittering hops. At the time, we used aroma hops. Was that a good decision? If that emergency were to arise again, should we substitute aroma hops again, or should we use dry hopping hops instead? As much as possible, we would want the product from the emergency-substituted batch to be as similar to the normal batch made with bittering hops.*

```{r, echo = FALSE, message = FALSE, eval = FALSE}
#asdfasdf group_by_summarize, hclust
hops_components <- read_csv("/Users/lucasbusta/Documents/Science/Website/thebustalab.github.io/R_For_Chemists/sample_data/hops_components.csv")
...
beer_analyzed2 <- runMatrixAnalysis(
  data = hops_components, # the data set to work on
  analysis = c("pca-ord"), # the analysis to conduct
  column_w_names_of_multiple_analytes = NULL, # a column with names of multiple analytes
  column_w_values_for_multiple_analytes = NULL, # a column with quantities measured for multiple analytes
  columns_w_values_for_single_analyte = colnames(hops_components)[5:12], # a column with quantities measured for a single analyte
  columns_w_additional_analyte_info = NULL, # a column with character or numeric information about analytes that was not "measured" as part of the experiment.
...
Mr. Porter: *Note to self: here we are interested not in comparing `total_oil` from different `hop_varieties`, but instead in comparing `total_oil` from hops with different `hop_brewing_usage`.*
...
```{r, echo = FALSE, message = FALSE, eval = FALSE}
mutate(hops_components, acids = alpha_acids + beta_acids) %>%
  ggplot(aes(x = hop_variety, y = acids)) + geom_col() +
  coord_flip()
```
...
```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), results="markup"}
knitr:::include_graphics('http://thebustalab.github.io/R_For_Chemists/figures/project.png', dpi = NA)
```

For your final project you will use the chemometric techniques we learned in class to either (i) conduct a detailed analysis of a large dataset or (ii) build a data analysis tool. You will then generate high quality figures to describe your analysis or tool, and you will build a manuscript around those figures (see workflow above).

For your manuscript, I suggest that you write the sections in the order of the chapters that follow below, paying close attention to the tips or suggestions given for each section.

While everyone's manuscripts will differ, I suggest aiming for the following criteria:
  * 4-6 figures (if you have more than this, consider using an appendix/supplementary information section)
  * 1 results section paragraph for each 1-2 figures
  * 3+ discussion paragraphs

<!-- in future: research questions, exploratory vs hypothesis-driven research -->

<!-- end -->

<!-- start Figures and Captions -->
...
* Objective: describe how the study was performed. Refer to the chapters of this book for details about the analysis techniques we've covered in class.

## Suggestions

* Create subsections according to experiments / techniques / sets of analyses.
* Describe methods used in sufficient detail so that another research lab with average experience could reproduce the results.
* For this project, describe where you got the data, how you cleaned it up, if you merged multiple datasets, etc.
<!-- end -->
...
* Objective: Walk your reader through your results.
...
* As you go: make notes of what should go into the introduction and discussion.

## Structure

* Key: number of optional sentences / **number of suggested sentences**: *purpose*: "example"

* Intro paragraph:
  + **1**: *Review the aim of the paper*: "In order to understand…"
  + **3-4**: *Combine methods summary with generalized summary of results to call out subsections*: "We used method X to quantify property Y of our study subject (section 3.1)"
...
* Each subsection paragraph:
  ...
  + 1: *Review methods specific to subsection*
    + *make sure verbs apply to the obj. in question*
  + **4-5**: *Results of that method*
  + **1-2**: *Conclusion from those results or concluding remark* "Thus, analysis X revealed that..."
<!-- end -->
...
* Objective: Use your data to argue for or against your or others' ideas.

<!-- * Tense: Where referring to data, past tense. When referring to literature, past perfect, active voice. -->
...
* Create paragraph outlines:
	...
	+ Identify "data features" in your figures, then possible conclusions those could lead to. Example:
		...
		+ "suggesting that either cities are a source of more pollution or that there is some other mechanism that concentrates pollution in cities."
	...
	+ Expand your "data feature" -> "conclusion" combinations with "supplementary information" or "literature information". Example:
		...
		+ "These direct observations are consistent with meta-analyses of previously published observations (Supplemental Figure 1), as well as with conclusions drawn by others in recent literature reviews (So and so et al., 2018; The other person et al., 2019)" (supplementary and literature information.)
...
* Write drafts of your paragraphs:

	+ Combine each of your "data feature" -> "supp/lit info" -> "conclusion" combinations into a single paragraph.
	+ Consider editing each paragraph so that it highlights what new contribution your data makes to the situation. Example (see italics):
...
* Edit your discussion section as a whole:
...
* Objective: to convey a short statement of the take-home messages of your study. What are the most important things that you want the reader to remember from your study? 

## Suggestions

* Summarize over-arching conclusions from each section of the paper (omit details from results or discussion)
    + Based on a general description of findings, use pros and cons to argue for, if possible, alternative hypotheses.
    + Suggest experiments to test these hypotheses.
...
* Objective: to prepare the reader by giving the reader sufficient background to understand the study as a whole. It therefore should only contain information pertinent to understanding the study and its broader significance. 
...
* Paragraph 1: Introduce the topic
  + Introduce a topic, ideally, an application of the research you will describe (grab attention of reader with an important fact or relationship).
  + State why the topic is important.
  + Describe what is known about the topic (at least, as pertains to the work at hand).
  + Identify a gap in knowledge: "despite research in this area, here is what we don't know about the topic."
  + List the negative things that will happen if we don't fill this gap in knowledge.
  
* Paragraph 2: Provide background information
  + Describe, in moderate detail, the background information (concepts, literature) relevant to the study.
  + End by saying how the details you just described relate to the application/topic described in the first paragraph.

* Paragraph 3: Objectives of this study
  + State the objective of this study.
  + Briefly describe what was done and the techniques or instruments used.
  + (optional - personal preference) State the conclusions from the work and what they mean for the application described in paragraph 1.
...
* Objective: prove a short summary of the entire study. 
...
* Purposes of an Abstract
  + Informs readers about the article’s content
  + Summarizes complex information in a clear, concise manner
  + Helps readers decide whether or not to read the article
  + Used in conferences to summarize what the speaker will say during his/her presentation

## Suggestions
  
* Use one or more well-developed, concise paragraphs that are able to stand alone.
* Use about 200 - 500 words (ideally no more than 400 words)
* Use an introduction-body-conclusion structure in which the parts of the paper are discussed in order:
  + Introduction (1-2 sentences): Describe the topic, the motivation, and overall purpose of the research (Why is this research interesting and important? What gap in our knowledge does it fill?)
  + Research Objective (1-2 sentences): Specific research objective, and potentially hypotheses/predictions
  + Methods (1-2 sentences): Very concise overview of the methods used to address the research questions
  + Results (1-3 sentences): Describe the major results (what you found) and interpretation of the results (what the results mean)
  + Conclusions (1-2 sentences): Synthesizes the major contributions of the study in light of the new information provided by the results of the study – what does it all mean? Why is the result important, What did we learn in the bigger picture from doing this study?)
...
* Objective: Capture attention and highlight the research question.

## Suggestions

* There are essentially two types of titles: descriptive titles and conclusive titles. 
  ...
  + If your manuscript is hypothesis driven research, consider using a conclusive title. For example:
    + "The Rice Gene "OsGL1-6" Is Involved in Leaf Cuticular Wax Accumulation and Drought Resistance"

* A good title should:
  + Be indicative of the content of the paper
  + Attract the interest of potential readers
  + Reflect whether the article is formal or casual
...
(base function). Use it to find things that are in a range or list. Some examples:
...
```{r}
# Find cars in the horsepower mid-range
mid_strength <- filter(mtcars, cyl %in% c(4, 8))
```
...
```{r loadRunMatrixAnalysis, eval = FALSE}
