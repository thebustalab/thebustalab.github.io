...
Filter the data set so only meausurements of free elements are shown. Remember, it's `==`, not `=`. What are the dimensions of the following dataset?
...
Make a plot that shows the water temperature of each lake. The x axis should be `park`, the y axis `water temp`. Add geom_violin() to the plot first, then geom_point(). Make the points size = 5. Color the points according to water_temp. Which park has the most uniformity among its lakes' temperatures?
...
Make a plot that uses geom_point(). Set the "shape" aesthetic of the points to 21, i.e. `geom_point(aes(...), shape = 21)`. This gives you access to new aesthetics: `fill` and `stroke`. It also changes the behaviour of the `color` aesthetic slightly. Here is an example:
...
ggplot(data = filter(lake_data, lake == "Lake_Narvakrak"), aes(x = lake, y = mg_per_L)) +
  geom_point(shape = 21, size = 10, color = "black", fill = "green", stroke = 3)
```

Now we have lots of aesthetics we can map to: x, y, size, color, fill, and stroke (leave shape set to 21). Make a plot that uses the whole dataset and all of these aesthetics. It's ok to map multiple aesthetics to the same variable if you want.

```{r, include = FALSE}
ggplot(data, aes(x = element, y = mg_per_L)) + 
  geom_point(shape = 21, )
```
