...
Let's illustrate this with an example. We're going to use a function from the `tidyverse` called `read_csv`. We'll use it to read some data from a path on our computer. This is a link to the original, untidied version of the data, but you can download a tidied version of the data by clicking here. We're going to import that tidied version (not the original, untidied version) using the `read_csv` command. We'll run `read_csv("<path_to_your_data>")`. Note the use of QUOTES! Those are necessary. Also make sure your path uses appropriate slashes for your operating system.

```{r}
read_csv("https://github.com/thebustalab/R_For_Chemists/sample_data/algae_data.csv")
```
...
## Read in the algae_chemistry data. The quotes are critical! ## PC = forward slash ## Mac = backward slash
  algae_chemistry <- read_csv("/Users/lucasbusta/Desktop/algae_data.csv")

## Display the contents of the dataset
  algae_chemistry

## Let's filter the dataset to only show abundances of fatty acids ("FAs")
