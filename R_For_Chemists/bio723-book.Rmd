...
We can see that this dataset is comprised of measuring various *analytes* (elements, water_temp, pH), across different *samples* lakes. We need to tell the `runMatrixAnalysis()` function how each column relates to this samples and analytes structure. See the image below for an explanation.

```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), results="markup"}
knitr::include_graphics('http://thebustalab.github.io/phylochemistry/figures/runMatrixAnalysis1.png', dpi = NA)
```

Let's try out our template:
...
runMatrixAnalysis(
    ...
    column_w_multiple_analyte_names = NULL,
    column_w_multiple_analyte_values = NULL,
    columns_w_single_analyte_values = NULL,
    columns_w_analyte_annotation_info = NULL,

    columns_for_sample_unique_ID = NULL,
    columns_w_sample_annotation_info = NULL
...
#### runMatrixAnalysis
    ...
    #' @param analysis	
    #' @param column_w_multiple_analyte_names
	#' @param column_w_multiple_analyte_values
	#' @param columns_w_single_analyte_values
	#' @param columns_w_analyte_annotation_info
    ...
    #' runMatrixAnalysis
		...
		runMatrixAnalysis <-	function(
									...
									column_w_multiple_analyte_names,
									column_w_multiple_analyte_values,
									columns_w_single_analyte_values,
									columns_w_analyte_annotation_info,

									columns_for_sample_unique_ID,
									columns_w_sample_annotation_info
			...
			# Needs to remove columns that are not included in input lists

			# Remove analyte annotation columns before pivoting

				if( length(columns_w_analyte_annotation_info) > 0 ) {
					analyte_annotation_free_data <- data[,-match(columns_w_analyte_annotation_info, colnames(data))]
				} else {
			...
			# If no pivot required, skip pivoting
				...
				if( length(column_w_multiple_analyte_names) == 0 & length(columns_w_single_analyte_values) >= 1 ) {
					data_wide <- analyte_annotation_free_data
					analyte_columns <- columns_w_single_analyte_values
				}

			# If pivoting require, pivot_wider any long-style data

				if( length(column_w_multiple_analyte_names) == 1 ) {
					data_wide <- pivot_wider(
						analyte_annotation_free_data,
						names_from = all_of(column_w_multiple_analyte_names),
						values_from = all_of(column_w_multiple_analyte_values)
					)
					analyte_columns <- unlist(unique(analyte_annotation_free_data[,colnames(analyte_annotation_free_data) == column_w_multiple_analyte_names]))
				} 

			# Add sample_unique_ID_column if necessary, or just change column name of existing sample_unique_ID column

				if( length(columns_for_sample_unique_ID) > 1 ) {
					sample_unique_IDs <- apply(
						data_wide[,match(columns_for_sample_unique_ID, colnames(data_wide))],
						1, paste, collapse = "_"
					)
					if( any(duplicated(sample_unique_IDs)) ) {stop("columns_for_sample_unique_ID specified do not lead to unique sample IDs")}
					data_wide$sample_unique_ID <- sample_unique_IDs
				} else {
					colnames(data_wide)[colnames(data_wide) == columns_for_sample_unique_ID] <- "sample_unique_ID"
					if( any(duplicated(data_wide$sample_unique_ID)) ) {stop("columns_for_sample_unique_ID specified do not lead to unique sample IDs")}
				}
			...
			# Clustering analysis
				...
				# Add back annotations to the output
					...
					clustering <- full_join(
						data_wide[,match(c(columns_w_sample_annotation_info, "sample_unique_ID"), colnames(data_wide))],
						clustering,
						by = "sample_unique_ID"
					)
