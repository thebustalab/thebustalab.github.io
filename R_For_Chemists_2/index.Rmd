...
```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), results="markup"}
knitr:::include_graphics('http://thebustalab.github.io/R_For_Chemists_2/figures/markdown_1.png', dpi = NA)
```
...
algae_data_data
ggplot(
  data = filter(algae_data_data, chemical_species == "essential_Aas"),
...
Use R to view the first few lines of the alaska_lake_data dataset.
...
library(tidyverse)
alaska_lake_data
ggplot(data = filter(alaska_lake_data, lake == "Lake_Narvakrak"), aes(x = lake, y = mg_per_L)) +
...
```{r, include = FALSE}
ggplot(filter(alaska_lake_data, element == "C"), aes(x = park, y = mg_per_L)) + 
