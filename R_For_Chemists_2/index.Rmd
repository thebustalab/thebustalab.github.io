...
With these paths, we can read in data using the `read_csv` command. We'll run `read_csv("<path_to_your_data>")`. Note the use of QUOTES `""`! Those are necessary. Also make sure your path uses the appropriate direction of slashes for your operating system. Finally, if you are on PC, you will need to include double backslashes instead of single backslashes when you type (or ideally paste) the path into your read_csv() command.
...
Neat! The pipe will become more important as our analyses become more sophisticated, which happens very quickly when we start working with summary statistics, as we shall now see...
