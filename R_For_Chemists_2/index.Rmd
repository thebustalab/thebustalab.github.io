...
Very good. Now let's talk about evaluating the quality of our model. For this... residuals



For this consider a rudimentary model that is just defined by the mean y value of the input data.

```{r}
ggplot(metabolomics_data) +
  geom_point(aes(x = ADP, y = AMP)) +
  geom_hline(aes(yintercept = mean(AMP)))
```

A pretty bad model, I agree. Let's create a measure of the distance between each point and the point predicted for that same x value on the model:

```{r}
sum(
  (metabolomics_data$ADP - mean(metabolomics_data$AMP))^2
, na.rm = TRUE)

ggplot(metabolomics_data) +
  geom_point(aes(x = ADP, y = AMP)) +
  geom_hline(aes(yintercept = mean(AMP))) +
  geom_segment(aes(x = ADP, y = AMP, xend = ADP, yend = mean(AMP)))
```

49.8! Wow. Let's call that the "total sum of the squares". Let's go back to our linear model and calculate the same thing:
...
15.39! Let's call that the "residual sum of the squares". Much better. Now we can calculate our R squared value. It is the 
