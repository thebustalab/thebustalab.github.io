...
local({
  ...
  if (lib_time > cache_time) {
    stop(
      ...
      "  (That forces a COLD render, which re-runs the live API chunks in ch13/ch14 — expected.)\n",
      "  To clear only from chapter 7 onward and keep the early chapters cached:\n\n",
      "      for n in $(seq 237 634); do rm -f _bookdown_files/index_cache/html/unnamed-chunk-${n}_*; done\n\n",
