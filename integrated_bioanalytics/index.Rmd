...
## Randomly split data into a traing set and a testing set
    metabolomics_data_split <- initial_split(metabolomics_data, prop = 3/4)
...
For nonmodel species transcriptome analysis, `transXpress` is recommended. `transXpress` uses the Trinity assembler by default, which can require ~300GB of free disk space to run. Depending on your machine, you may also need to make some modifications to `transXpress` for it to run properly. First, make sure to add conda-forge::ncurses as a dependency in the trinity_utils.yaml file to avoid errors with the script not being able to detect the samtools installation. The trinity_utils.yaml file should look like:
...
Also add an explicit pip dependencey to the tmhmm.yaml file:
`
channels:
  - conda-forge
  - bioconda
...
Also, you may also need to scale your memory usage in trinity:
`
...
Finally, it's possible that you will need to downgrade numpy for tmhmm to work (make sure you do this in the transxpress env):
`
