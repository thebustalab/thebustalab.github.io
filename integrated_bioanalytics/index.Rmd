...
The above code builds our random forest model. It's output provides both the model itself and key components indicating hte performance and configuration of the model. Here's a breakdown of each part of the output:

`$metrics` provides detailed results of model performance across different combinations of the Random Forest parameters mtry (the number of variables randomly sampled as candidates at each split) and trees (the number of trees in the forest). For each combination, it shows:
- mtry and trees: The specific values used in that model configuration.
...
`$model` shows the configuration of the Random Forest model that was trained, including:
- Preprocessor: Specifies any preprocessing steps applied to the data before modeling. Here, it indicates that no preprocessing steps (0 Recipe Steps) were applied.
- Model: Describes the Random Forest model (rand_forest()) used, including its settings like the number of trees, sample size, number of variables, mtry, target node size, variable importance mode, split rule, and the out-of-bag (OOB) prediction error.

`$rmse_or_percent_accuracy` shows the overall percent accuracy of the model, which is the best accuracy achieved on the test set across the tested configurations. For regression models, it shows RMSE while for classification models, it shows percent accuracy on the test set.

`$fold_cross_validation` reiterates the number of folds used in cross-validation.

We can thus inspect the performance of the model based on the specific parameters used during configuration:
...
rfc <- buildModel(
  ...
  optimization_parameters = list(n_vars_at_split = seq(30), n_trees = seq(4,5,1))
)
...
```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), results="markup"}
knitr:::include_graphics('https://thebustalab.github.io/integrated_bioanalytics/images/wood_smoke.jpg', dpi = NA)
```
...
bloom_example_pics <- ggplot(data = data.frame(x = c(0,1), y = c(0.5,0.5))) +
  ...
  annotation_custom(
      rasterGrob(
          readPNG(
              "https://thebustalab.github.io/integrated_bioanalytics/images/homology.png"
          ), interpolate=TRUE
