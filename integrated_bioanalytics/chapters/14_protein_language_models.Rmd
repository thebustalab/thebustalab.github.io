...
```{r, echo = FALSE}
## RENDER-TIME INPUT, not something a student runs. The chunk above (eval = FALSE) is what produced
## it, via the NVIDIA embedding API -- so this file must NOT be regenerated casually.
##
## 2026-09-22: a COLD render died here with `gzfile(): cannot open the connection`. That was the first
## time this line had actually executed in months -- it had been a chunk-cache hit, so a broken read
## was invisible for as long as the cache survived. Resolve the path defensively and, if it still
## fails, say WHY (working directory, what is actually in data/, open-connection count) instead of
## repeating a four-word error.
## THE WORKING DIRECTORY FOR A CHAPTER CHUNK IS `chapters/`, NOT THE BOOK ROOT. Measured in a real
## render 2026-09-22: `getwd()` here is
## .../integrated_bioanalytics/chapters, so the correct relative path to the book's data/ dir is
## "../data/...". A bare "data/..." looks right, is wrong, and was masked for months by the chunk
## cache. Candidates are tried in order so the chunk works from either directory.
.pe_candidates <- c("../data/protein_embeddings_example.rds",
                    "data/protein_embeddings_example.rds")
.pe_path <- .pe_candidates[file.exists(.pe_candidates)][1]
if (is.na(.pe_path)) {
  stop("ch14: embeddings file not found.\n  wd: ", getwd(),
       "\n  tried: ", paste(.pe_candidates, collapse = " , "),
       "\n  ../data/ contains: ",
       paste(head(list.files("../data"), 20), collapse = ", "), call. = FALSE)
}
all_sequences_embedded <- tryCatch(
  readRDS(.pe_path),
  error = function(e) stop(
    "ch14: the embeddings file EXISTS but could not be read.\n  wd: ", getwd(),
    "\n  path: ", normalizePath(.pe_path, mustWork = FALSE),
    "\n  size: ", file.info(.pe_path)$size, " bytes",
    "\n  open connections: ", nrow(showConnections(all = FALSE)),
