...
```{r, include = FALSE}
## DO NOT put a bare source("https://thebustalab.github.io/phylochemistry/phylochemistry.R") back here.
## `include = FALSE` hides this chunk's output; it does NOT stop it evaluating. The whole book knits in
## ONE session, and ch3 runs before ch7, so a bare re-source replaced the working-copy library that
## _common.R had just loaded with the PREVIOUS RELEASE -- and ch7's runMatrixAnalysis(analysis = "dist",
## output_format = "long") then returned a bare `dist` that ggplot() could not fortify. That broke the
## 2026-09-22 deploy repeatedly, with an error ~12000 lines from its cause. _common.R already
## bootstraps the library for every chapter, so this is a no-op; it only acts if something upstream
## failed, and then it prefers the working copy.
if (!exists("algae_data")) {
  .pc3 <- file.path("..", "phylochemistry", "phylochemistry.R")
  if (file.exists(.pc3)) source(.pc3) else
